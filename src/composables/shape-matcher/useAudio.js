...
// Pre-warm daftar suara browser agar tidak kosong saat dipanggil
...
export function useAudio() {
  ...
  const playSoundEffect = (type) => {
    ...
    try {
      ...
      if (type === 'correct') {
        if (!successAudio) {
          successAudio = new Audio('/audio/shape-matcher/success.mp3');
  ...
  const playNarrator = (text) => {
    // 1. Cek pengaturan sakelar narator & dukungan browser
    if (!store.isNarratorOn || !('speechSynthesis' in window)) return;

    try {
      // 2. Hentikan narasi sebelumnya & lepas status paused/stuck
      window.speechSynthesis.cancel();
      if (window.speechSynthesis.paused) {
        window.speechSynthesis.resume();
      }

      // 3. Inisialisasi SpeechSynthesisUtterance
      const utterance = new SpeechSynthesisUtterance(text);
      utterance.lang = 'id-ID';
      utterance.rate = 0.85;
      utterance.pitch = 1.15;

      // 4. Pilih suara Bahasa Indonesia jika tersedia
      ...
      // 5. Eksekusi pemutaran dengan sedikit delay agar cancel() selesai berproses
      setTimeout(() => {
        window.speechSynthesis.speak(utterance);
      }, 50);
    } catch (err) {
      console.error('Gagal memutar suara narator:', err);
    }
  };
  ...
  const playMusic = () => {
    ...
    if (!bgMusicAudio) {
      bgMusicAudio = new Audio('/audio/shape-matcher/bg-music.mp3');
